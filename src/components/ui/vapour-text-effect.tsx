...
import { FaComments } from "react-icons/fa"; // NEW

// ── UPDATED: wrapper that keeps the vapor text looping, mobile-safe ──
export const Component = () => {
  // responsive font size for the animated heading
  const [fontPx, setFontPx] = useState<number>(70);

  useEffect(() => {
    const calc = () => {
      const w = typeof window !== "undefined" ? window.innerWidth : 1280;
      let size = 70;           // desktop default
      if (w < 360) size = 30;
      else if (w < 420) size = 30;
      else if (w < 480) size = 42;
      else if (w < 640) size = 48;   // sm
      else if (w < 768) size = 56;   // md-
      else size = 70;                // md+
      setFontPx(size);
    };
    calc();
  ...
  // wrapper height derived from font size to avoid clipping
  const headingBoxHeight = Math.round(fontPx * 1.5);

  return (
    <section className="bg-black text-white min-h-screen flex items-center">
      <div className="mx-auto max-w-5xl w-full px-6 sm:px-8 text-center py-16">
        ...
        <div className="flex items-center justify-center" style={{ height: `${headingBoxHeight}px` }}>
          <VaporizeTextCycle
            ...
            spread={5}
            density={5}
            animation={{ vaporizeDuration: 2, fadeInDuration: 0.5, waitDuration: 0.5 }}
