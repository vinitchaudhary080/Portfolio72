...
export default function CardBepro() {
  const thumbs = [thumb1, thumb2, thumb3, thumb4, thumb5, thumb6, thumb7];
  const looped = [...thumbs, ...thumbs];
  const tags = ["Sports", "Mobile", "MVP"];
  const caseStudyHref = "/bepro/bepro-app-design";

  return (
    <article className="px-6 mb-16" aria-labelledby="bepro-title">
      <div
        className="
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-[60%_40%]">
          ...
          <figure className="relative overflow-hidden h-[220px] sm:h-[260px] md:h-auto">
            <img
              src={mainImage}
              alt="BePro cricket app — home screen mockup"
              className="
          ...
          <div className="relative p-5 sm:p-6 md:p-8 flex flex-col">
            ...
            <h3 id="bepro-title" className="text-white text-lg sm:text-xl md:text-2xl font-light leading-snug">
              <Link to={caseStudyHref} className="hover:underline underline-offset-4">
                BePro App
              </Link>
              <br />
              <span className="text-white/90">Play, Track &amp; Relive Your Matches</span>
            </h3>

            <p className="text-white/75 text-sm sm:text-base leading-relaxed mt-3">
              BePro is a cricket app built for local players, street cricketers, and
              stadium-level teams. Create matches, track live scores, run tournaments,
              and maintain performance records—just like professional cricket.
            </p>

            <p className="mt-3 text-xs text-white/55">Product UX • Mobile</p>
            ...
            <div className="mt-5">
              <Link
                ...
                "
                aria-label="View BePro app design case study"
              >
                View Project
